...
int main(void)
{
    ...
    A_f = malloc(N_i * sizeof(*A_f));
    B_f = malloc(N_i * sizeof(*B_f));
    C_f = malloc(N_i * sizeof(*C_f));
    D_f = calloc(N_i, sizeof(*D_f));
    D_f_avx = calloc(N_i, sizeof(*D_f_avx));
    ...
    for (int i = 0; i < N_f / N_i; i++)
        f_vector_op(A_f, B_f, C_f, D_f, N_i);
    ...
    gettimeofday(&t1, NULL);
    for (int i = 0; i < N_f / N_i; i++)
        f_vector_op_avx(A_f, B_f, C_f, D_f_avx, N_i);
    gettimeofday(&t2, NULL);
    t_opt = MSTIME(t1, t2);
    ...
    C_i = calloc(N_f, sizeof(*C_i));
    C_i_avx = calloc(N_f, sizeof(*C_i_avx));
    ...
    gettimeofday(&t1, NULL);
    i_vector_op(A_i, B_i, C_i, N_f);
    gettimeofday(&t2, NULL);
    t_neopt = MSTIME(t1, t2);
    printf("Runtime for non-AVX code: %fs\n", t_neopt);

    gettimeofday(&t1, NULL);
    i_vector_op_avx(A_i, B_i, C_i_avx, N_f);
    gettimeofday(&t2, NULL);
    t_opt = MSTIME(t1, t2);
    printf("Runtime for AVX code: %fs\n", t_opt);
